...
try {
  ...
  try {
    $createPostResponse = Invoke-WebRequest -Uri 'http://localhost:3000/posts' -Method POST -Headers @{
  ...
  try {
    $profileResponse = Invoke-WebRequest -Uri 'http://localhost:3000/users/profile' -Method GET -Headers @{
      'Authorization'="Bearer $token"
    }
  ...
  Write-Host "`nTest: Generate Content (POST /openai/generate)" -ForegroundColor Yellow
  try {
    $aiResponse = Invoke-WebRequest -Uri 'http://localhost:3000/openai/generate' -Method POST -Headers @{
      'Content-Type'='application/json'
      'Authorization'="Bearer $token"
    } -Body '{"topic": "Social media marketing tips", "platform": "twitter", "tone": "professional"}'
